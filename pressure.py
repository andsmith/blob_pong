...
class PressureField(CenterScalarField):
    ...
    def set_incompressible(self, v_new, v_const, dt, phase='gas'):
        """
        ...
        if phase == 'gas':
            ...
            for row in range(self.n_cells[0]):
                for col in range(self.n_cells[1]):
                    ...
                    A_vals.append(-n_neighbors / dx**2)
                    # B vector term:
                    B_vals.append(vel_diverg / dx)

        else:
            raise NotImplementedError("Liquid phase not implemented yet.")
        
        # Create the sparse matrix A:
        A = csr_matrix(( A_vals,(A_row_inds, A_col_inds)), shape=(A_size, A_size), dtype=np.float64)
        # Create the B vector:
        B = np.array(B_vals, dtype=np.float64)
        p = solve(A, B)
        # Set the pressure field to the solution:
